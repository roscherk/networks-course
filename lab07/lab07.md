...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - 10.8.1.2:39284
   <image src="images/a_1.png" width=500>
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence number 0 (raw 839251616)
   - SYN-сегмент определяется соответствующим флагом
   <image src="images/a_3.png" width=500>
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence number 0 (raw 2015799553)
   - в поле подтверждения в SYNACK-сегменте хранится 1
   - Acknowledgment number = Sequence number + 1
   - SYNACK-сегмент определяется соотв. флагами
   <image src="images/a_4.png" width=500>
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - У меня пресловутый TCP-сегмент разбился на два с Sequence number 1 и 3
   <image src="images/a_5f.png" width=500>
   <image src="images/a_5s.png" width=500>
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Порядковые номера: 3, 707, 2031, 3355, 4679, 6003
   - Время отправки: 6.514438765, 6.514511993, 6.514522062, 6.514533554, 6.514592224, 6.514615618
   - Время получения ACK: 6.647202357, 6.647492715, 6.647502613, 6.647873322, 6.648462563, 6.648509051
   - RTT: 0.132763592, 0.132980722, 0.132980551, 0.133339768, 0.133870339, 0.133893433
   <image src="images/a_6_1.png" width=500>
   <image src="images/a_6_2.png" width=500>
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Пропускная способность равна примерно 60 кб/с
   - Всего передали 8140 байт со средним RTT = 0.133304734 $\implies$ пропускная способность $\approx 8140 / 0.133304734 \approx 61063$ b/s $\approx 60$ kb/s
...
#### Скрин
<image src="images/b.png" width=500>
...
#### Решение
Чтобы загрузка канала была больше $98%$, нужно занимать больше $98%$ BDP.

BDP = R × RTT = $10^9$ бит/с \cdot $0.03$ с = $3 \cdot 10^7$ бит

Размер окна $0.98 \cdot 3 \cdot 10^7$ бит = $2.94 \cdot 10^7$ бит

Значит, пакетов должно быть не меньше $\frac{2.94 \cdot 10^7} / 12000 = 2450$

Ответ: размер окна должен быть не меньше $2450$ пакетов.
